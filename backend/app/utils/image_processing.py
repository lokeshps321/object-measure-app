...
Return ONLY the JSON. No markdown. No explanation."""
        ...
        for i, obj in enumerate(objects_data):
            ...
            # Refine bbox with OpenCV contours inside Gemini region for tight box
            pad = 15
            rx, ry = max(0, x - pad), max(0, y - pad)
            rx2, ry2 = min(w, x + bw + pad), min(h, y + bh + pad)
            ...
            if roi.size > 0:
                ...
                cnts2, _ = cv2.findContours(cv2.morphologyEx(otsu, cv2.MORPH_CLOSE, np.ones((5,5),np.uint8)), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                best, best_a = None, 0
                roi_a = roi.shape[0] * roi.shape[1]
                for c in list(cnts) + list(cnts2):
                    a = cv2.contourArea(c)
                    if roi_a * 0.05 < a < roi_a * 0.99 and a > best_a:
                        best_a, best = a, c
                if best is not None:
                    cx_r, cy_r, cw_r, ch_r = cv2.boundingRect(best)
                    x, y, bw, bh = rx + cx_r, ry + cy_r, cw_r, ch_r
