...
def measure_objects_local(
    ...
    max_area = w * h * 0.95   # not more than 95% of frame
    img_cx, img_cy = w // 2, h // 2  # image center
    detected_regions = []
    measured_objects = []
    annotated = image.copy()

    # Score each contour by area AND proximity to image center
    scored = []
    for contour in all_contours[:50]:
        ...
        dist = ((cx_c - img_cx)**2 + (cy_c - img_cy)**2) ** 0.5
        max_dist = (w**2 + h**2) ** 0.5
        center_score = 1.0 - (dist / max_dist)  # 1 = at center, 0 = at corner
        area_score = area / (w * h)
        # Combined score: center matters most, then area
        score = center_score * 0.7 + area_score * 0.3
        scored.append((score, contour, x, y, bw, bh))

    # Sort by combined score (best first)
    scored.sort(key=lambda s: s[0], reverse=True)

    for score, contour, x, y, bw, bh in scored:
        # Duplicate check
        is_dup = False
